...
namespace Lean
{
	...
	public class LeanTouch : MonoBehaviour
	{
		...
		public static void MoveCamera(Transform transform, Vector2 lastScreenPos, Vector2 deltaPosition, Vector2 minPos, Vector2 maxPos)
		{
			if (transform != null)
			{
				if (deltaPosition.x != 0.0f || deltaPosition.y != 0.0f)
				{
					/*
					 = transform.position;
					Debug.Log( (float)modPosition.x + (float)deltaPosition.x );

					modPosition.x = Mathf.Clamp( ((float)modPosition.x + (float)deltaPosition.x) / 10f, (float)minPos.x, (float)maxPos.x);
					modPosition.y = Mathf.Clamp( ((float)modPosition.y + (float)deltaPosition.y) / 10f, (float)minPos.y, (float)maxPos.y);
					transform.position = modPosition;
*/
					// Find current screen position of world position
					var  screenPosition = Camera.main.WorldToScreenPoint(deltaPosition);
					
					// Modify screen position
					Vector3 newDeltaPostion = new Vector3( deltaPosition.x/10f, deltaPosition.y/10f); 

					screenPosition -= (Vector3)newDeltaPostion;
					Vector3 modPosition = Camera.main.ScreenToWorldPoint( screenPosition );
					
					modPosition.x = Mathf.Clamp( transform.position.x - ((float)modPosition.x /100f), (float)minPos.x, (float)maxPos.x);
					modPosition.y = Mathf.Clamp( transform.position.y - ((float)modPosition.y /100f), (float)minPos.y, (float)maxPos.y);
					modPosition.z = transform.position.z;
